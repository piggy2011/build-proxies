#!/bin/bash
# ============================================================
# argosbx_refined_v1.2.sh
# RareCloud multi-protocol auto-config generator
...
# ---------- Generate xr.json (xray-core with Argo) ----------
...
echo -e "${Green}All 22 (plus Argo) subscription links generated → ${LINK_FILE}${Font}"

# ---------- Generate UFW rules ----------
cat > "${WORKDIR}/ufw_rules.sh" <<'EOF'
#!/bin/bash
# UFW rules for RareCloud proxy services
...
# ---------- Print all subscription links ----------
echo -e "${Yellow}=========== Generated Subscription Links ===========${Font}"
cat "${LINK_FILE}"
echo -e "${Green}=====================================================${Font}"

echo -e "${Green}✔ All configuration and subscription files created in ${WORKDIR}${Font}"
